...
#  before_filter :login_required
  ...
  def create
    ...
    respond_to do |format|
      if @event.save
        flash[:success] = t('success_event_created')
  ...
  def update
    respond_to do |format|
      if @event.update_attributes(params[:event])
        flash[:notice] = t('notice_event_updated')
        format.html { redirect_to(@event) }
        format.xml  { head :ok }
      else
  ...
  def attend
    if @event.attend(current_person)
      flash[:notice] = t('notice_you_are_attending_this_event')
      redirect_to @event
    else
      flash[:error] = t('error_you_can_only_attend_once')
      redirect_to @event
    end
  end
  ...
  def unattend
    if @event.unattend(current_person)
      flash[:notice] = t('notice_you_are_not_attending')
      redirect_to @event
    else
      flash[:error] = t('error_you_are_not_attending')
      redirect_to @event
    end
  end
  ...
  private
    ...
    def in_progress
      flash[:notice] = t('notice_in_progress')
