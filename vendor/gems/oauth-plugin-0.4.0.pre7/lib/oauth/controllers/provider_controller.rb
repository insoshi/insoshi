  ...
  module Controllers
    ...
    module ProviderController
      ...
      def oauth2_authorize_code
        @client_application = ClientApplication.find_by_key params[:client_id]
        if nil == @client_application
          render :action => "authorize_failure"
          return
        end
        if request.post?
          @redirect_url = URI.parse(params[:redirect_uri] || @client_application.callback_url)
          if user_authorizes_token_without_capabilities?
            @verification_code = Oauth2Verifier.create :client_application=>@client_application, :person=>current_person, :scope => params[:scope], :callback_url=>@redirect_url.to_s
      ...
      def oauth2_authorize_token
        ...
        if request.post?
          @redirect_url = URI.parse(params[:redirect_uri] || @client_application.callback_url)
          if user_authorizes_token_without_capabilities?
            @token  = Oauth2Token.create :client_application=>@client_application, :person=>current_person, :scope=>params[:scope]
            unless @redirect_url.to_s.blank?
